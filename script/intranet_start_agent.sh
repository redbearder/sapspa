...
#            echo "download Python "$PYTHON_VERSION
            mv ${BASE_DIR}script/download/Python-${PYTHON_VERSION}.tar.xz ~/.pyenv/sources/${PYTHON_VERSION}/Python-${PYTHON_VERSION}.tar.xz
...
function install_agent_requirements()
{
  ...
  cd ${BASE_DIR}/script/download/pip
  pip3 install `ls -l |awk '{print $9}'`
}
